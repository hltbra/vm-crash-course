...
#define PUSH_ARG 0x8
#define INC_SP 0x9
#define PUSH_LOCAL 0xA
#define POP_LOCAL 0xB
...
// helper macro
#define LOCAL_VAR(vm, arg) *((vm).fp + (arg))
...
int main(int argc, char *argv[]) {
    ...
    while (!stack_empty(&vm)) {
        ...
        if (operator == PUSH) {
            ...
            restore_state(&vm);
            push(&vm, result);
        } else if (operator == PUSH_ARG) {
            // last two addresses on the stack are IP and SP;
            // before that there is the local data (decrement of 2).
            // SP always points to the next addr, thus, we need to decrement one more.
            push(&vm, *(vm.fp - 3 - operand));
            vm.ip++;
        } else if (operator == INC_SP) {
            // sp always point to the next bucket
            vm.sp += operand + 1;
            vm.ip++;
        } else if (operator == POP_LOCAL) {
            LOCAL_VAR(vm, operand) = pop(&vm);
            vm.ip++;
        } else if (operator == PUSH_LOCAL) {
            push(&vm, LOCAL_VAR(vm, operand));
            vm.ip++;
        }
    }
    dump(&vm);
